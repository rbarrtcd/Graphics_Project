...
struct MyBot {
	...
	void initialize() {
        ...
        jointMatricesID = glGetUniformLocation(programID, "jointMatrix");
	}
	...
	void render(glm::mat4 cameraMatrix) {
        ...
        for (const auto& mat : skinObjects[0].jointMatrices) {
            const float* matPtr = glm::value_ptr(mat); // Pointer to the raw data of the glm::mat4
            jointMatrixData.insert(jointMatrixData.end(), matPtr, matPtr + 16); // Copy 16 floats
        }
        glUniformMatrix4fv(jointMatricesID, skinObjects[0].jointMatrices.size(), GL_FALSE, jointMatrixData.data());
